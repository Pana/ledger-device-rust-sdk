...
enum MainCommand {
    ...
    Build {
        ...
        #[clap(short, long)]
        #[clap(help = concat!(
            "Copy output files (.hex, .elf, .apdu) to standard ledger app build tree (inspired from C SDK). ",
            "This is useful when integrating with other ledger tools that expect the output to be in a specific location.",
        ))]
...
fn main() {
    ...
    match cli.command {
        ...
        MainCommand::Build {
            ...
            build_app(d, a, cli.use_prebuilt, cli.hex_next_to_json, s, r);
        }
    }
}
    ...
    if standard_build_tree {
        ...
        let copy_binaries_to_standard_path = |dir: &PathBuf| {
            ...
            if let Err(err) = fs::create_dir_all(&dir) {
                eprintln!(
                    "Error creating standard directory : {}",
                    err
                );
            }
            println!(
